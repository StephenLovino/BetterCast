...
//
// BetterCast is free and unlicensed, so whether somebody donated is not
// knowable from inside the app: "I already donated" is an honour-system button
// that silences the prompt for good, not a verified state. Every launch asks,
// the first one included, until that button is pressed - nothing else stops
// it. (The macOS prompt skips the first launch; this one deliberately does not.)
//
